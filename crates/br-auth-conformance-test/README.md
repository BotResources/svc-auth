...
| The harness is `br-test-harness` **v1.1.0** with `FabricTestNats`, not a raw `SpawnedNats` | v1.1.0 is the harness release coupled to `br-rust-common` v1.1.0 (matches the contract's `br-core-*` pins). `FabricTestNats` is the house provisioner: it stands up the real `nats-server`, get-or-creates the fixed `PUBLISHED_LANGUAGE` bucket, and hands back the **real lib** publisher/reader — `async-nats` stays confined to the harness, never touched by this crate. |
