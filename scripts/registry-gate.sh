...
if [[ -z "${BR_REGISTRY_KEY:-}" ]]; then
    echo "::error::registry gate: BR_REGISTRY_KEY is not set — the sealed-and-not-implemented release law cannot be verified for ${SERVICE} ${VERSION}. The gate refuses rather than passes: an unverifiable law is a broken one. Provision the repository secret on BotResources/svc-auth (check it was not deleted, and that this workflow is not running in a context that withholds secrets)."
    exit 1
fi
...
case "${VERDICT}" in
    ELIGIBLE)
        ...
        ;;
    UNSUPPORTED)
        echo "::error::registry gate: ${BR_REGISTRY_URL} does not expose servicesValidateImplementationTarget — the sealed-and-not-implemented law cannot be verified for ${SERVICE} ${VERSION}. Production has run svc-services >= 0.4 since 2026-07-27, so this means the registry was rolled back, or BR_REGISTRY_URL points somewhere that is not production."
        exit 1
        ;;
    "REFUSED patch_already_implemented"*)
        ...
        echo "OK   ${SERVICE} ${VERSION}: already implemented in the registry — treating this as an idempotent re-run of the CD that shipped it (${IMAGE_REF}:${VERSION}). Whether that image exists is GHCR's answer, not the registry's: the publish job's tag gate and docker's push idempotence cover it."
        exit 0
        ;;
    "REFUSED not_found"*)
        echo "::error::registry gate: ${SERVICE} has no PatchVersion ${VERSION} in the Services registry. Create and seal ${VERSION} in the registry (services-ops) BEFORE bumping the crate — the registry version is the release authorization."
        exit 1
        ;;
    "REFUSED patch_not_sealed"*)
        echo "::error::registry gate: ${SERVICE} ${VERSION} exists in the Services registry but is NOT SEALED. Seal it (servicesSealMinorVersion / servicesSealPatch) before publishing."
        exit 1
        ;;
    *)
