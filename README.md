...
| The registry gate (`scripts/registry-gate.sh`) has no pass-by-default path | A missing `BR_REGISTRY_KEY`, a network failure, or a registry not exposing the query all fail the release. A release law that a missing secret (or a registry rollback) can silently switch off is not a law. |
| The registry service id is a committed UUID (`registry.toml`), doubly declared in `scripts/service-meta.sh` | A service has no other stable handle: by-name lookup would silently retarget releases on a rename, and the by-name entry point is closed to machine keys anyway. The cross-check between the two committed copies stops a pasted-into-the-wrong-repo id from writing onto another service's patch. Versions below the service are addressed by the Cargo.toml numbers — no patch id is ever committed. |
| Registry transport is `curl` only, payloads through files, keys never in argv | Cloudflare 403s other user agents on `botresources.ai`; file-borne payloads dodge the 128 KiB per-argument kernel limit; the bearer key reaches only the HTTP header, never argv or the log. |
| `registry-docs.sh` poses BOTH documents as empty strings, still `procedural: true` | svc-auth serves no GraphQL and owns no database, and an empty document is a legitimate photo of that reality. Posing them (rather than skipping) is what counts toward the registry's automatic `isImplemented` flip. |
| `registry-docs.sh` carries no SDL/DB extraction machinery, and fails loudly if `service-meta.sh` ever declares a surface | The be-botresources.ai reference extracts the SDL from the built binary (`env -i <binary> schema`) and recomposes the DB schema on an ephemeral pinned Postgres. svc-auth has neither surface, so that machinery is dead weight here — but a declared surface with no extraction path must refuse, never pose a false empty document. Port the reference paths before flipping the declaration. |
| The gate's third `image-ref` argument turns `patch_already_implemented` into a pass | Its presence declares the caller to be the CD run about to publish that ref, making a re-triggered run (tag push failure recovery) an idempotent no-op. Without it — a human, or PR-time CI — the refusal stands: a published version is immutable. |
| The `registry-implement` flip probe is advisory and inverted | Reading `isImplemented` back needs an org-gated read a machine key is refused, so the job re-asks the machine-admitted eligibility question: a `patch_already_implemented` refusal IS the success signal. By then the image is on GHCR, so a red job would report a failed release that in fact succeeded — only the image-record write fails hard. |
...
Releases are gated on the production Services registry: the bumped version must exist there as a **sealed, not-implemented** PatchVersion (create and seal it before bumping the crate). CI validates the target at PR time, CD re-validates before the build and again immediately before the push, poses the patch's two procedural documents (both empty — no GraphQL, no database), and records the published image ref after the push so the patch flips to implemented. The pieces live in `scripts/registry-{gate,docs,implement}.sh` + `scripts/service-meta.sh`, with the service's registry UUID committed in `registry.toml`.
